...
## Certificates and knowledge checks

Completing a course's last lesson (`markLessonComplete` in
`src/lib/progress.ts`) does three things in sequence: stamps
`Enrollment.completedAt`, issues a certificate, and emails the learner —
the certificate/email step is wrapped in a try/catch so a bug there can
never turn the learner's own "Mark complete" click into an error; the
enrollment is already completed by the time either runs.

- **`src/lib/certificate.ts`** is the one place certificate data and
  rendering live — `getCertificateData` (learner name, course title,
  hours from `estimatedMinutes`, completion date, a verification code
  deterministically derived from the enrollment id via SHA-256, and the
  issuer's name/license from `CERTIFICATE_ISSUER_NAME`/
  `CERTIFICATE_ISSUER_LICENSE_NUMBER`), `renderCertificatePdf`, and
  `issueCertificate` (idempotent: an already-issued certificate is
  returned as-is, never regenerated). There's no plausible default for a
  real credentialed issuer's name and license number, so leaving those
  env vars unset makes every certificate say so plainly rather than
  showing invented credentials.
- **`/certificates/[enrollmentId]`** is a print-styled page (no
  `AppShell` chrome — a learner printing it or saving it as a PDF from
  the browser shouldn't get the site nav on the page), owner-only or
  ADMIN. **`GET /api/certificates/[enrollmentId]/pdf`** issues the
  certificate if it hasn't been already and streams the stored PDF —
  same ownership rule. The HTML page and the PDF both render from the
  same `getCertificateData` call; the PDF is drawn directly with
  `pdfkit` rather than converting the HTML page to PDF, since that would
  need a headless-browser dependency (Playwright/Puppeteer, with a
  bundled Chromium) this app doesn't otherwise have — a documented
  judgment call, the same shape as earlier ones in this project.
  Cert PDFs are stored via a new `StorageService.saveGeneratedFile`
  (server-generated bytes, not a browser upload — `saveFile` and
  `saveCoverImage` both stayed as they were) under their own
  `generated` prefix, and served through the owner-checked PDF route
  rather than `/api/files` or `/api/course-covers`, since a certificate
  is personal, not a lesson resource or a public marketing asset.
  `CLINICIAN`-track certificates carry a fixed disclaimer line verbatim
  (`CLINICIAN_DISCLAIMER`): this documents completion of a self-paced
  program, not APA-approved continuing education.
- **The completion email** (`src/lib/completionEmails.ts`) links the
  certificate and recommends one next course — `findRecommendedNextCourse`
  picks a `PUBLISHED` course on the *same track* the learner isn't
  already enrolled in, other than the one they just finished; `null`
  rather than a guess when nothing fits.
- **Quiz/QuizQuestion** are optional knowledge checks attached to a
  `Module` (one quiz per module, `@@unique([moduleId])`), multiple
  choice with an explanation shown after answering. They're untimed,
  unlimited attempts, never scored, and never gate lesson progress or
  the certificate — `QuizPlayer.tsx` is a purely client-side component
  with nothing saved to the database; picking a different option after
  answering is always allowed, and `Enrollment.completedAt` never checks
  quiz state. A quiz surfaces through the existing `LessonType.QUIZ`
  lesson slot (previously an honest "not available yet" placeholder,
  documented as a loose end in earlier phases) — a module's quiz is
  reachable only if that module also has a `QUIZ`-type lesson.
